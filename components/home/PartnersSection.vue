...
// Dynamically fetch only the current locale, re-fetch if locale changes
const { data: partnersSection } = await useAsyncData(
    `partners-section-${locale.value}`,
    () => queryContent(`/${locale.value}/home/partners-section`).findOne(),
    { watch: [locale] }
);
</script>
